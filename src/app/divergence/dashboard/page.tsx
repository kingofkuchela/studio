...
import { GitCompareArrows, ArrowUp, ArrowDown, Minus, RotateCcw, Target, Bot, CheckCheck, GitCommitHorizontal, Ban, ArrowRight, LogIn, LogOut } from 'lucide-react';
    ...
    return (
        <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Badge variant="secondary">{stats.tradeCount} Trades</Badge>
            </CardHeader>
            <CardContent className="flex-grow flex flex-col justify-around">
                <div className="text-center">
                    <p className="text-sm text-muted-foreground">Net P&L</p>
                    <p className={cn("font-bold text-3xl", stats.totalPnl > 0 ? "text-green-600" : stats.totalPnl < 0 ? "text-red-600" : "text-foreground")}>
                ...
                <div className="grid grid-cols-2 gap-4 text-center mt-4 pt-4 border-t">
                    <div>
                        ...
                        <p className="text-xs font-semibold text-green-500">{formatCurrency(stats.grossProfit)}</p>
                    </div>
                    <div>
                        <p className="text-sm text-muted-foreground">Losses</p>
                        <p className="font-bold text-lg text-red-600">{stats.losses}</p>
                        <p className="text-xs font-semibold text-red-500">{formatCurrency(stats.grossLoss > 0 ? -stats.grossLoss : 0)}</p>
                    </div>
                </div>
            </CardContent>
        </Card>
...
const rulesFollowedFilterOptions: { id: RulesFollowedStatus; name: RulesFollowedStatus }[] = [
  ...
  { id: 'Divergence Flow', name: 'Divergence Flow' },
];
...
export default function DivergenceDashboardPage() {
    ...
    const divergenceStats = useMemo((): DivergenceStats => {
        const allEdgesCombined = [...edges.real, ...edges.theoretical];
        ...
        // Corrected Partial Divergence Logic
        const partialDivergenceRealTrades = filteredTrades.real.filter(t =>
            (t.executionMode === 'both' && t.closeMode !== 'both') || 
            (t.executionMode !== 'both' && t.closeMode === 'both')
        );
        const partialDivergenceRealTradeIds = new Set(partialDivergenceRealTrades.map(t => t.id));
    
        const partialDivergenceTheoTrades = filteredTrades.theoretical.filter(t => partialDivergenceRealTradeIds.has(t.id));
    ...
    }, [filteredTrades, edges.real, edges.theoretical]);

    const entryExecutionScore = useMemo(() => {
        const alignedCount = divergenceStats.aligned.tradeCount;
        const partialCount = divergenceStats.partialReal.tradeCount;
        ...
        if (totalOpportunities === 0) {
            return { percentage: 0, executed: 0, total: 0 };
        }

        const percentage = (executedTrades / totalOpportunities) * 100;
        return {
                    ...
                    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col">
                        ...
                        <CardContent className="flex-grow flex flex-col justify-center items-center">
                            {entryExecutionScore.total > 0 ? (
                                ...
                                <p className="text-sm text-center text-muted-foreground p-4">No data to calculate execution score.</p>
                            )}
                        </CardContent>
                    </Card>
                </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Card className="shadow-lg">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Analysis of entry points and deviations.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="h-[200px] flex items-center justify-center text-muted-foreground">
                                <p>Entry execution data will be displayed here.</p>
                            </div>
                        </CardContent>
                    </Card>
                     <Card className="shadow-lg">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Analysis of exit points and deviations.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                             <div className="h-[200px] flex items-center justify-center text-muted-foreground">
                                <p>Exit execution data will be displayed here.</p>
                            </div>
                        </CardContent>
                    </Card>
                </div>
