...
export default function CancelOrderDialog({ isOpen, onClose, onConfirm, order }: CancelOrderDialogProps) {
  ...
  const handleSubmit = (values: FormValues) => {
    onConfirm(order.id, values.reason);
    onClose();
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Cancel Order: {order.symbol}</DialogTitle>
          <DialogDescription>
            You must provide a valid reason to cancel this pending order.
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Why do you want to cancel this entry?</FormLabel>
                  <FormControl><Textarea placeholder="Describe your reasoning for this cancellation..." {...field} /></FormControl>
                  <FormMessage />
